...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		// // Other settings
		// set('downscroll', Options.downScroll);
		// set('framerate', Options.framerate);
		// set('ghostTapping', Options.ghostTapping);
		// //set('hideHud', ClientPrefs.data.hideHud);
		// //set('timeBarType', ClientPrefs.data.timeBarType);
		// //set('scoreZoom', ClientPrefs.data.scoreZoom);
		// //set('cameraZoomOnBeat', ClientPrefs.data.camZooms);
		// set('flashingLights', Options.flashingMenu);
		...
		*/
		...
		var thing:String = scriptName;
		try{
			var isString:Bool = !FileSystem.exists(thing);
			var result:Dynamic = null;
			if(!isString)
				result = LuaL.dofile(lua, thing);
			else
				result = LuaL.dostring(lua, thing);
	...
	public function call(func:String, args:Array<Dynamic>):Dynamic {
		...
		trace(lastCalledFunction);
		trace(lastCalledScript);
		...
		try {
			...
			trace('ma balls after getglobal');
			var type:Int = Lua.type(lua, -1);
			trace('ma balls after type');

			if (type != Lua.LUA_TFUNCTION) {
				trace('ma balls have no function');
			...
			trace('ma balls before converit');
			for (arg in args) Convert.toLua(lua, arg);
			trace('ma balls after connverting');
			trace('${lua} + ${args.length} + 1 + 0');
			var status:Int = 0;
			trace('ma balls after status thing');

			// Checks if it's not successful, then show a error.
			if (status != Lua.LUA_OK) {
				trace('ma balls not okay');
			...
			trace('ma balls after the before result');
			var result:Dynamic = cast Convert.fromLua(lua, -1);
			trace('ma balls after results ' + result);
			if (result == null) result = LuaUtils.Function_Continue;

			Lua.pop(lua, 1);
			trace('popped a wheelie');
			if(closed) stop();
			trace('we now return with created function${func}');
			return result;
		}
		catch (e:Dynamic) {
			trace(e);
	...
	{
		if(!Options.gameplayShaders) return false;
